...
本リポジトリは上流 [tsuyopon123/cube-j1-mqtt](https://github.com/tsuyopon123/cube-j1-mqtt) のフォーク的派生であり、上流互換性を保ちつつ観測性・ローカル運用性・遠隔保守性を強化することを目的とする。
...
Cube J1 上のランタイムは Python 2.7 で、pip による追加ライブラリ導入はサポートしない。`mqtt_bridge.py` および同居スクリプトは Python 2.7 stdlib のみで完結させる。MQTT クライアントもシリアル制御も HTTP サーバも自前実装を継承する。テストコードは Python 3 で書いてよいが、被テストコードは 2.7 互換を維持する。
...
### IV. 計測ロジックを観測性・管理機能の追加で劣化させない
ログ・診断 publish・組み込み Web UI の追加は、Wi-SUN ポーリングや ECHONET Lite パース等の計測パスをブロックしてはならない。診断送信失敗・HTTP リクエスト処理は計測値送信を妨げない。HTTP サーバはメインループとは別 thread で動作させ、I/O 待ちが計測パスのレイテンシを実測可能な単位で増やさない。

### V. テスト駆動 (TDD) で改修する
新規追加・修正は t_wada スタイルの Red→Green→Refactor で進める。被テストロジック（ECHONET Lite フレーム組立/パース、診断状態の集約、JSON ログフォーマッタ、HTTP ハンドラ等）は副作用と分離し、ハードウェア依存のないユニットテストを書く。シリアル/MQTT/HTTP は I/O 境界で抽象化する。

### VI. 組み込み管理 Web UI は Optional + LAN 限定 + 認証必須
bridge プロセスに HTTP サーバを同居させて Cube J1 自体を遠隔から設定変更できる。導入時の原則は以下:
- 既定で **無効**（`config.json` の `admin_ui_enabled` キーで明示有効化）
- LAN 内のみで運用する想定。**外部公開はしない**（Cube J1 を直接インターネット露出させない）
- 認証は **Basic Auth**（`stdlib` のみで実装可能）。`admin_user` / `admin_password` を `config.json` で指定
- 機密ファイルへの書き込みは temp ファイル経由のアトミック書き込み（temp → `os.rename`）
- HTTP サーバの障害（ポート使用中・例外）は bridge 本体の動作を止めない

## Hardware & Runtime Constraints

- ターゲット機器: NextDrive Cube J1（Wi-SUN モジュール BP35C0 内蔵、armhf 系 Linux、Python 2.7、Android init）
...
- 組み込み Web UI ポート（既定）: TCP 8080（`config.json` `admin_ui_port` で変更可）
...
- **障害時挙動**: MQTT 切断中も計測は継続し、queue に保持して再接続時に flush する。診断値も同様に扱う。HTTP サーバが落ちても計測は続ける。
- **ローカル運用先**: 受信側 (Mosquitto + Home Assistant + Telegraf) は lab-ub01.home.arpa 上の Docker Compose で運用する。Grafana Cloud への送信は Telegraf 経由（bridge から直接外部 HTTP を呼ばない）。
...
**Version**: 2.0.0 | **Ratified**: 2026-06-19 | **Last Amended**: 2026-06-19
