...
- 重要 EVENT を固定キーで publish (HA discovery 互換):
  - `sk_event_22_total` (PANA 成功)
  - `sk_event_24_total` (PANA 失敗 / 認証失敗)
  - `sk_event_25_total` (応答完了通知)
  - `sk_event_26_total` (PANA 再認証要求)
  - `sk_event_28_total` (PANA セッション切断)
  - `sk_event_29_total` (PANA セッションタイムアウト)
  - `sk_event_32_total` (ARIB アクティブスキャン完了)
  - `sk_event_33_total` (ARIB アクティブスキャン開始)
